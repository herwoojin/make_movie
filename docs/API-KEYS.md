...
4. 옆의 **저장하고 키 확인**을 누릅니다 → `✅ 쓸 수 있는 키입니다 (AIza…abcd) — gemini-… 모델로 번역합니다` 가 나오면 끝
5. 같은 칸 위의 **기본 번역 엔진**을 `Gemini (내 API 키)`로 두기

> 키는 **입력하는 순간 저장**됩니다(저장 버튼이 따로 없어도 됩니다). 칸 아래 `저장됨: AIza…abcd` 의 앞뒤 4자가
> AI Studio 키 목록의 키와 같은지 보세요. 키를 바꿀 때는 **칸을 모두 지운 뒤** 붙여넣으세요 — 옛 키 뒤에 이어 붙으면
> "키 2개가 이어 붙어 있음" 경고가 뜹니다(번역할 때는 되는 키를 찾아 씁니다).
> 키는 **사이트 주소별로 따로** 저장됩니다. `localhost` 에서 넣은 키는 `1u2v.netlify.app` 에서 보이지 않으니, 쓰는 사이트에서 넣어 주세요.
