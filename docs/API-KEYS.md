...
- 쓸 모델은 **자동으로 고릅니다**. 구글이 옛 모델을 내려도(예: `gemini-2.0-flash`는 2026-06-01 종료) 내 키로 쓸 수 있는 최신 Flash 모델로 알아서 넘어갑니다.
- 무료 사용량을 넘기면 다른 모델로 한 번 더 해 보고, 그래도 안 되면 "무료 사용량을 다 썼습니다" 안내가 뜹니다. 1분쯤 뒤 **번역 다시 시도**를 누르거나 DeepL로 바꾸세요.
- 번역이 중간에 실패해도 **원어 자막은 저장돼 있어** 음성 인식을 다시 하지 않습니다. 결과 표의 **번역 다시 시도**를 누르면 번역만 다시 합니다.
- Google AI Studio에서 키에 **웹사이트 제한**을 걸었다면 `https://1u2v.netlify.app/*` 를 넣어 두세요. 빠져 있으면 "키로 번역을 쓸 수 없습니다" 안내가 뜹니다.
...
> **배포 사이트(https://1u2v.netlify.app)에서 연결할 때**: 처음 한 번 Chrome이
> **"이 기기의 다른 앱 및 서비스에 액세스"** 를 허용할지 묻습니다. **허용**을 눌러야 연결됩니다.
> 실수로 차단했다면: 주소창 왼쪽 사이트 정보 아이콘 → **로컬 네트워크 액세스 → 허용** (설정 화면에도 안내가 뜹니다).
> `localhost:3000`으로 연 개발 서버에서는 묻지 않습니다.
